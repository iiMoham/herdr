...
questions. Everything else goes through the herdr CLI at $HERDR_BIN_PATH.
...
def start():
    ...
    ok, payload, raw = herdr(worktree_create_args(workspace_id, branch, config["base"]))
    if not ok:
    ...
    for kind, value in layout_steps(config):
        if kind == "run_root":
            ok, payload, raw = herdr(["pane", "run", root_id, value])
        else:
            ok, payload, raw = herdr(
            ...
            if ok and value["command"] and new_pane:
                ok, payload, raw = herdr(["pane", "run", new_pane, value["command"]])
        if not ok:
...
def finish():
    ...
    ok, payload, raw = herdr(["worktree", "removal-check", "--workspace", workspace_id])
    nested = (
    ...
    flags = []
    while True:
        ok, payload, raw = herdr(["worktree", "remove", "--workspace", workspace_id, *flags])
        if ok:
            return 0
