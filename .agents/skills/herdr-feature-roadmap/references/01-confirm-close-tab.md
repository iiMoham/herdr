...
every process in it immediately. Herdr only confirms when the close would remove a workspace
...
- `config_reference_check.py` passes; `herdr --default-config` lists the key commented out.
