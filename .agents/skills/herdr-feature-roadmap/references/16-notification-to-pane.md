...
Sources: upstream discussions #4531 "System notification click should focus the originating Herdr
...
- Clicking a Herdr system notification (macOS `terminal-notifier`; Linux `notify-send` actions where
...
- New client-control surface so automation can do the same: `herdr client focus-pane --machine
...
- Machine selection: `src/client/shell/endpoints.rs`, `src/client/shell/endpoint_navigation.rs`,
  `~/.local/state/herdr/client/endpoint-selection.json` handling.
