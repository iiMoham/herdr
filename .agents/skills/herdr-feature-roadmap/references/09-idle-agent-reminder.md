...
- When an agent has been idle for `remind_after_minutes` (default 50), shows
  `herdr notification show "<agent> idle 50m" --body "<workspace> / <tab>" --sound request` once per
  idle period.
...
Plugin. It composes existing events + CLI and needs no core state. See `herdr-plugin-authoring`.
...
1. A startup hook that launches a supervised background process (`nohup`-style) which subscribes to
   events (`herdr` event subscription over the socket) and sleeps until the next deadline. Must exit
...
- Pure logic tests for the idle tracker (state transitions, one reminder per idle period, snooze,
  agent filter) runnable without Herdr.
- Smoke test in a throwaway session with `remind_after_minutes` lowered (allow a seconds override for
  ...
  (`herdr pane report-agent` / integration docs) rather than a paid agent.
...
README with install (`herdr plugin link`), config, limits (per-server, lost on server stop unless
