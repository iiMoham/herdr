...
project's own session, keep the old one running. Herdr has `session attach`, but nothing moves an
already attached client to another named session, so pickers must nest clients or ask the user to
detach manually.

## Behavior

- `herdr client switch-session <name> [--create] [--cwd PATH]`, usable from custom commands and
