...
Source: upstream discussion #4527 "Pane balancing (pane.balance / herdr pane balance)".
...
- CLI: `herdr pane balance [--tab ID]` (defaults to the caller's tab via `HERDR_TAB_ID`, then focused tab).
...
- Keybinding: checklist in `herdr-fork-dev`.
