...
description: Decide whether a herdr feature belongs in a plugin instead of core, then build, link, test, and document a herdr plugin (herdr-plugin.toml manifest, actions, event hooks, startup hooks, panes, link handlers). Use for roadmap features classified as plugins.
---

# Authoring herdr plugins for the fork
...
- The feature composes existing CLI/API calls (`herdr workspace|tab|pane|agent|worktree|notification …`).
...
`herdr-fork-dev`), then the plugin. Ask the user before adding the core primitive.
