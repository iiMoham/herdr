...
| Worktree | `worktree.list`, `worktree.create`, `worktree.open`, `worktree.remove`, `worktree.removal_check`, `worktree.remove_discarding_nested` |
| Tab | `tab.create`, `tab.list`, `tab.get`, `tab.focus`, `tab.rename`, `tab.move`, `tab.close`, `tab.set_input_sync` |
...
Applications and plugins should emit the standard Kitty graphics protocol through
their pane's terminal output. Herdr manages image placement, clipping, and
visibility in compatible outer terminals.

The external socket pane-overlay API has been removed: `pane.graphics.info`,
`pane.graphics.set`, `pane.graphics.clear`, and `pane.graphics.stream` are no
longer available. There is no replacement socket image API. Internal wire
compatibility with older servers does not restore these public methods.
...
Worktree methods manage Git checkouts as Herdr workspaces. `worktree.create` creates a checkout and returns the new `workspace`, `tab`, `root_pane`, and `worktree` records. If the requested branch already exists locally, it checks out that branch; otherwise it creates the branch from the requested base or `HEAD`. `worktree.open` opens an existing checkout or returns the already-open workspace. `worktree.remove` runs `git worktree remove` against a linked child workspace and never deletes the branch. It first checks for Git repositories nested inside the checkout and refuses with `worktree_nested_repositories_at_risk` (or `worktree_removal_check_incomplete` for a checkout too large to verify) when any of them has uncommitted changes, untracked files, stashes, or commits on no remote, including when `force` is true. `worktree.removal_check` returns `{workspace_id, checkout_path, complete, nested: [{path, relative_path, kind, summary, modified, untracked, stashes, unpushed_commits}]}` without removing anything. `worktree.remove_discarding_nested` removes anyway, but only when its `nested_paths` lists every repository that is still at risk; newly at-risk work is refused again.
...
`pane.output_matched`, `pane.agent_status_changed`, `pane.scroll_changed`, and `pane.output_changed`.
...
`workspace_id`, and `revision` whenever the pane's screen content changed since
the last event. It is sampled on the subscription cadence (every 100 ms), so a
burst of output produces one event carrying the latest revision, and existing
content at subscribe time does not emit. Set `"include_text": true` to attach a
`read` of the new content; `source` (default `recent`), `lines`, and
`strip_ansi` work as in `pane.read`. Revisions increase but are opaque and not
contiguous: compare them with `content_revision` from `pane.get` or `revision`
from `pane.read` to tell whether content you hold is current. The subscription
stops emitting when the pane closes; watch `pane.closed` to end it. It is not
available as a plugin event hook.

```json
{"id":"sub_1","method":"events.subscribe","params":{"subscriptions":[{"type":"pane.output_changed","pane_id":"w1:p2","include_text":true,"lines":40}]}}
```
