...
The default is `midnight-neon`: cyan marks focus and working agents, magenta marks agents that need you, green marks finished work, and idle agents stay a calm periwinkle. Herdr never paints over your terminal's background, so for the full look set your terminal profile's background to `#0b1020`. Its light sibling is `neon-day`. Search `theme.name` in the [Config reference](/docs/config-reference/) for every built-in theme. Use `terminal` when you want Herdr UI colors to follow your host terminal's ANSI palette.
...
The expanded sidebar opens with an animated pixel herd: one sheep per agent across all machines, colored by the agent's state. The herd walks while agents work (faster with more working agents), a sheep whose agent needs you shows a blinking `!`, and the herd sleeps when nothing is working. A status line below it counts working agents and agents that need you. It animates at `animation_fps` frames per second (default 24, 1-30) until you close the window, redraws only when a frame changes, and hides when the sidebar is collapsed, shorter than 16 rows, or in the mobile layout. Turn it off with:
...
Set `tab_bar_padding = 1` or `2` under `[ui]` to leave blank rows between the tab row and the panes, similar to tmux `status 2`. The padding sits below a top tab row and above a bottom one, disappears while the tab row is hidden, and yields first on very short terminals. The default is `0`.
...
On macOS, `system` tries `terminal-notifier` first and falls back to `/usr/bin/osascript` when it is unavailable or fails. The fallback appears as Script Editor in Notification Center and cannot activate the hosting terminal. Install `terminal-notifier` with `brew install terminal-notifier`. For a supported, detected terminal, it can activate the terminal app when you click the notification. Clicking a notification that came from a pane also takes the Herdr window that showed it to that workspace, tab, and pane, switching to the saved SSH machine the agent runs on when needed; other Herdr windows stay where they are. The AppleScript fallback cannot react to clicks, and Linux and Windows notifications do not open panes. Alternatively, choose `terminal` to let a supported outer terminal own the notification.
