...
`notification open-target` is the command a clicked system notification runs (macOS with `terminal-notifier`): it asks the Herdr window that showed the notification, through that window's private socket, to open the pane the notification came from. Tokens are single-use and only the newest 32 notifications per window stay clickable.
...
herdr worktree removal-check --workspace ID [--trust-repository]
```
...
Before deleting anything, Herdr looks for Git repositories nested inside the checkout, such as sub-repos the parent repository ignores, submodules, or other worktrees. Git cannot see them, so a clean-looking checkout can still hold uncommitted changes, untracked files, stashes, or commits that exist on no remote. If any nested repository would lose work, removal is refused, even with force, and Herdr lists what is at risk. Nested repositories whose work is all committed and pushed do not block removal, and worktrees of repositories outside the checkout only count their uncommitted changes, because their commits live elsewhere. The refusal (`worktree_nested_repositories_at_risk`, or `worktree_removal_check_incomplete` when the checkout is too large to verify) names each repository, for example `frontend: 1 unpushed commit, 1 untracked path`. `--discard-nested` prints what will be discarded and deletes it anyway. `worktree removal-check` reports the same findings as JSON without removing anything.
...
herdr tab sync <on|off|toggle> [tab_id]
```
...
`tab sync` turns input sync on, off, or toggles it for a tab; without `tab_id` it uses `HERDR_TAB_ID`, so it works from inside a pane. While sync is on, typing and paste sent from an attached client to any pane in the tab reach every pane in that tab, like tmux `synchronize-panes`. Each pane receives the input encoded for its own terminal modes, such as bracketed paste. Mouse input, popups, and CLI/API input (`pane send-text`, `pane send-keys`, `agent prompt`) are never broadcast. Sync is shared by every attached client, shown as an `S` next to the tab name, and resets to off when the server restarts. `tab list` and `tab get` include `"input_sync": true` for synced tabs.
...
Stream every change instead of waiting for one match:

```bash
herdr pane watch <pane_id> [--text] [--source visible|recent|recent-unwrapped] [--lines N] [--raw]
```

`pane watch` prints one JSON line per content change, at most every 100 ms: `pane_id`, `workspace_id`, and an opaque, increasing `revision`. `--text` adds a `read` of the new content, shaped like `pane read --format text` output. It runs until interrupted, the reader closes (for example `| head -5`), or the server stops; it keeps running quietly if the pane closes, so pair it with `pane.closed` when that matters. `pane get` reports the current value as `content_revision`, and `pane read` reports the revision of the content it returned.
