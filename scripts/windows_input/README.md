...
Win32 only from captured runtime evidence. A missing host, unreachable required
size, or unavailable assertion exits `2`; an assertion or cleanup failure exits
`1`. Inspect the retained `report.json` for exact observations.
