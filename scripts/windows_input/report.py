...
def qualification_matrix(result):
    ...
    def cell(case_ids, width, path, modes, channels=required_channels, required_widths=None):
    ...
    for name, case_ids, width in groups:
        ...
        if release:
            ...
            table.append((name, cell(case_ids, width, "herdr", herdr_modes, {"stable"}, required_widths),
...
def main():
    ...
    if args.command == "report":
        ...
        if len(failures) > 10:
            print(f"... {len(failures) - 10} more failures in report.json")
